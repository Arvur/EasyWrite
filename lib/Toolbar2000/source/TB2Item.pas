...
uses
  MMSYSTEM, TB2Consts, TB2Common, IMM, TB2Acc, Types;
...
procedure TTBView.MouseWheel(WheelDelta, X, Y: Integer); {vb+}
  ...
  begin
    if not SystemParametersInfo(SPI_GETWHEELSCROLLLINES, 0, @Result, 0) then
      Result := 3; { From SDK: "The default value is 3" }
  end;

var
  WheelSrollLines: Integer;
  ScrollLines: Integer;
          ...
          case Msg.message of
            ...
            WM_MOUSEWHEEL:
              begin
                ...
                begin
                  P := Viewer.ScreenToClient(Msg.pt);
                  Viewer.MouseWheel(Smallint(LongRec(Msg.wParam).Hi), P.X, P.Y, Handled);
                end;
                if not Handled and (View <> nil) then
                begin
                  P := SmallPointToPoint(TSmallPoint(Msg.lParam));
                  Windows.ScreenToClient(Msg.hwnd, P);
                  View.MouseWheel(Smallint(LongRec(Msg.wParam).Hi), P.X, P.Y);
                end;
              end;
            {vb+end}
