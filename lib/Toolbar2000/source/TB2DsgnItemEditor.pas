...

{
  ...
  $jrsoftware: tb2k/Source/TB2DsgnItemEditor.pas,v 1.57 2006/01/09 19:22:07 jr Exp $
}
...
type
  TTBItemEditForm = class(TDesignWindow)
    ...
    procedure TImportActionClick(Sender: TObject);
  private
...
uses
  TypInfo, CommCtrl, TB2Version, TB2Common, TB2DsgnConverter,
    TB2DsgnActionImporter; // TB2DsgnActionImporter is NEW, added by Warren Postma, modified by Arvur
...
procedure TTBItemEditForm.TImportActionClick(Sender: TObject);
begin
  //-----------------------------
  // Invoke NEW Action Importer!
  //-----------------------------
  if FSelParentItem = nil then Exit;
  DoActionImport( FSelParentItem, FParentComponent.Owner);
end;
