...
uses
  ...
  {ComCtrls, Menus;} {vb-}
  ComCtrls, Menus, MMSystem, Types; {vb+}
...
begin
  ...
  while not (Value in AvailModes) do
    Value := Pred(Value);
  if (Value = amSysDefault) and not SysParamEnabled(SPI_GETMENUANIMATION) then {rl+}
    Value := amNone;                                                           {rl+}
...
procedure FixPlaySoundThreadProc(Param: Pointer); stdcall;
begin
  PlaySound(nil, 0, 0);
end;
...
var
  ...
  ThreadId: Cardinal;
begin
  if (Win32Platform = VER_PLATFORM_WIN32_NT) and CheckWin32Version(5) then
  begin
    ThreadHandle := CreateThread(nil, SizeOf(LongWord)* 4,
      @FixPlaySoundThreadProc, nil, CREATE_SUSPENDED, ThreadId);
...
initialization
  // Problems with DLLs  FixPlaySoundDelay; {vb+}
  {CurrentTheme := nil;} {vb-}
